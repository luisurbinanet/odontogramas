...
<script type="text/javascript">
    $(function() {
        ...
        $("#agregarEvol").click(function() {
            if($("#formEvol input[name='hidden-tags50']").val()!="" 
                && $("#formEvol input[name='fechaE']").val()!=""
                && $("#formEvol input[name='reciboE']").val()!=""
                && $("#formEvol input[name='valorE']").val()!=""){
                $.ajax({
                    type: 'POST',
                    ...
                    success: function(ide) {
                        setTimeout(function() {
                            $("<tr>"
                                ...
                                +"<td>"+$('#reciboE').val()+"</td>"
                                +"<td>"+$('#valorE').val()+"</td>"
                                +"<td>"+ $("#formEvol input[name='hidden-tags50']").val()+"</td>"
                                +"<td class='action icon16'><a class='icon-remove eliminarEvolucion' href='#eliminarEvolucion?ide="+ide+"' title='Eliminar'></a></td>" 
                                +"</tr>").insertBefore('#tablaEvol tr:last');
                            $('#formEvol').each (function(){
                                this.reset();
                            });
                            $(".tagManager3").tagsManager('empty');
                            $("#agregarEvol").button('reset');
                        }, 500);
        ...
        $(".eliminarEvolucion").live("click",function(e){
            var $this=$(this);
            var href = $(this).attr("href").split("?");
            if(confirm("¿Desea eliminar esta evolucion?")) {
                $.ajax({
                    type: 'POST',
                    url: "<%=request.getContextPath()%>/formController?action=eliminarEvolucion",
                    data: ""+ href[1],
                    success: function() {
                        setTimeout(function() {
                            $($this).parent('td').parent('tr').remove();
                ...
                <form method="post" id="formEvol">
                    <fieldset>
                        ...
                        <table class="table table-striped table-bordered table-condensed" id="tablaEvol" >
                            ...
                            <tbody>
                                <c:choose>
                                    <c:when test="${evoluciones.getRowCount()!= 0}">
                                        <c:set var="idActualX" value="${evoluciones.getRowsByIndex()[0][0]}"></c:set>
                                        <c:set var="tratamientosX2" value=""></c:set>
                                        <c:forEach items="${evoluciones.rowsByIndex}" var="evolucion" varStatus="status">
                                            <c:choose>
                                                <c:when test="${idActualX==evolucion[0]}">
                                                    <c:set var="tratamientosX2" value="${tratamientosX2} ${evolucion[4]}"></c:set>            
                                                    <c:choose>
                                                        <c:when test="${evoluciones.getRowsByIndex()[status.index+1][0]==null}">
                                                            <tr>
                                                                <td>${evolucion[1]}</td>
                                                                <td>${evolucion[2]}</td>
                                                                <td>${evolucion[3]}</td>
                                                                <td>${tratamientosX2}</td>
                                                                <td class="action icon16">
                                                                    <a class="icon-remove eliminarEvolucion" href="#eliminarEvolucion?ide=${idActualX}" title="Eliminar"></a>
                                                                </td>
                                                            </tr> 
                                                        </c:when>
                                                    </c:choose>
                                                ...
                                                <c:otherwise>
                                                    ...
                                                    <c:set var="idActualX" value="${evolucion[0]}"></c:set>
                                                    <c:set var="tratamientosX2" value="${evolucion[4]}"></c:set> 
                                                </c:otherwise>
                                            </c:choose>
                                ...
                                <tr>
                                    ...
                                    <td><input type="text" name="valorE" id="valorE" class="input-medium"></td>
                                    <td><input type="text" name="tags50" autocomplete="off" placeholder="Tratamiento" class="tagManager3"/></td>
                                </tr>
                            </tbody>
                        </table>

                        <button autocomplete="off" data-loading-text="Guardando cambios..." data-original-title="Agregar evolución" id="agregarEvol" type="button" style="margin-top: 18px;margin-left: 20px;" class="btn">Agregar evoluci&oacute;n</button> 
                                        ...
                                        <c:forEach items="${controles.rowsByIndex}" var="control" varStatus="status">
                                            <c:choose>
                                                <c:when test="${idActual==control[0]}">
                                                    ...
                                                    <c:choose>
                                                        <c:when test="${controles.getRowsByIndex()[status.index+1][0]==null}">
                                                            <tr>
                                                                <td>${control[1]}</td>
                                                                <td>${tratamientosX}</td>
                                                                <td class="action icon16">
